...
import { z } from 'zod';
import { presentClarificationFormInputSchema } from '@/lib/clarification-form';
...
function inferExplicitPriorityPool(messages: any[]): 'practitioner' | 'scholar' | null {
    ...
    const asksForPractitioners = /\bpractitioner(s)?\b|\bindustry\b|\bprofessional(s)?\b|\bexpert(s)?\b|\bfounder(s)?\b|\bstartup\b/.test(recentText);
    const asksForScholars = /\bscholar(s)?\b|\bacademic(s)?\b|\bresearch(er)?(s)?\b|\bprofessor(s)?\b|\bphd(s)?\b|\bfaculty\b/.test(recentText);
...
TONE AND VOICE (strict):
- Do not use first-person pronouns or self-reference (I, me, my, mine, myself, "I'd", "I've"). Write in neutral third person, imperative, or direct statements about the lab or the results.
- Avoid empty enthusiasm, cheerleading, or generic praise ("Great topic!", "Love this!", "That's fascinating!", "right in your wheelhouse", "I'd be happy to help", "Exciting work!").
- Eliminate generic chatbot fluff and "helpful intro" phrases (e.g., "To find the best collaborators, a few quick details:", "Let me help you with that", "Sure, I can find..."). Open with substance or a direct next step, not a reaction or a set-up phrase.
- Be concise, factual, and professional. Do not sound like a generic chatbot.

COMMUNICATION GUIDELINES:
- If current intent is "chat" (as determined by sub-inference), respond naturally WITHOUT tools. If they asked for a match but their request was too vague (e.g. "for my thesis" but no topic), prefer calling the tool "present_clarification_form" with 1–3 questions total and 1–3 preset options per question (the UI always includes an "Other" field per question) instead of long bullet lists in plain text. If a single short question is enough, a brief text ask is acceptable.
- ONLY trigger matching (calling ${enforcedDecision.priorityPool === 'practitioner' ? 'find_practitioners' : 'find_scholars'}) when the intent is "match" AND the user has given enough concrete detail (or has submitted clarification answers in a follow-up message).
- When the user message starts with "Clarification answers:" (bullet lines of question→choice in plain language), or legacy "[Clarification form answers]" JSON, treat those answers as authoritative constraints for the next search or reply.
...
- For each recommended candidate, provide a DEEP, personalized explanation (2-3 sentences max) OUTSIDE the card explaining exactly why their work aligns with the user's project—without praise, hype, or first-person phrasing.
...
${JSON.stringify(userProfile, null, 2)}`;
        ...
        const result = streamText({
            ...
            tools: {
                present_clarification_form: tool({
                    description:
                        'Present a structured clarification form when the user\'s goal is ambiguous or more detail is needed before searching. Use for any situation that benefits from structured choices—not only matching. At most 3 questions per call; each question has 1–3 preset options; the client always renders an additional "Other" text field per question. Keep title/intro strictly minimal or omit them; do not use generic filler like "a few quick details".',
                    inputSchema: presentClarificationFormInputSchema,
                    execute: async (input) => {
